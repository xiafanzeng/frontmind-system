...
11. 看板模块导入沿用原服务端预检；凭证绑定操作账号、客户、模块、修订号和文件哈希，并通过数据库 nonce 原子消费防止重放。客户可直接修改自己的看板、问题和应答逻辑，并直接重置知识库；工程师按已分配项目操作。工单提交、审批、附件与官网内容工单模板接口已移除。生产环境必须配置 `FRONTMIND_DASHBOARD_IMPORT_PREFLIGHT_SECRET`。
