...
export default function KnowledgeWorkspaceStatus({ progress }: { progress: KnowledgeBaseProgressDto | null }) {
  ...
  const message = reset
    ...
    : progress.packageAllowed ? "节点已处理完成，可以更新知识库。"
    : progress.packageState === "attention_required" ? "正文已完成，成果包暂时无法生成；已完成内容仍可阅读。"
    : progress.packageState === "preparing" || progress.packageState === "retrying" ? "正文已完成，正在准备成果包。"
    : progress.build.status === "published" ? "当前知识库已更新。"
