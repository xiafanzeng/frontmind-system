...
export default function PublishingMediaLibraryPage({
  ...
  const articleVersionId =
    ...
    flow?.resources?.find((resource) => resource.kind === "article_version")
      ?.id;
  const [entry, chooseEntry] = usePublishingChoice(
    "mediaEntry",
    draftId || articleVersionId || flow?.selections?.mediaSelection
      ? "catalog"
      : "",
  );
  const [confirmedMediaJson, , acceptConfirmedMedia] = usePublishingChoice(
    "mediaConfirmedSelection",
  );
  const confirmedMedia = useMemo(() => {
    try {
      const rows = JSON.parse(confirmedMediaJson);
        ...
        ? rows
            .filter(
              (
                ...
                typeof item.priceTenThousandths === "string" &&
                /^\d+$/u.test(item.priceTenThousandths),
            )
            .slice(0, 20)
        : [];
    } catch {
  ...
  const [mediaStep, chooseMediaStep, acceptMediaStep] =
    usePublishingChoice("mediaStep");
  const articleChooserOpen = mediaStep === "article";
  const setArticleChooserOpen = (open: boolean) =>
    chooseMediaStep(open ? "article" : "catalog");
  const [chosenVersion, setChosenVersion] =
    usePublishingChoice("mediaChosenVersion");
  const [boundDraft, setBoundDraft] = useState<PublicationDraft>();
  const [confirmedDraft, setConfirmedDraft] = useState<PublicationDraft>();
  const outputDraftId = (boundDraft ?? confirmedDraft)?.id;
  ...
  useEffect(() => {
    let active = true;
    if (outputDraftId)
      void publishingHandoffRecordId(`media-publishing-${outputDraftId}`).then(
        (recordId) => {
          if (active) setHandoffReceipt({ draftId: outputDraftId, recordId });
        },
      );
    return () => {
      active = false;
  ...
  const publishingTarget =
    handoffReceipt?.draftId === outputDraftId
      ? flow?.records?.find(
          (record) =>
            record.id === handoffReceipt?.recordId &&
            record.status === "completed" &&
            record.targetTask?.agentId === "publishing",
        )?.targetTask
      : undefined;
  ...
  useEffect(() => {
    ...
    if (flow) {
      setBoundDraft(draftQuery.data);
      setConfirmedDraft(draftQuery.data);
    }
    setSelected(
  ...
  usePublishingSummary({
    ...
    outputs:
      (boundDraft ?? confirmedDraft)
        ? [
            {
              id: `media-draft:${(boundDraft ?? confirmedDraft)!.id}`,
              title: `${(boundDraft ?? confirmedDraft)!.items.length} 家媒体与稿件已确认`,
              description: (boundDraft ?? confirmedDraft)!.articleTitle,
              type: "投放选择",
              version: (boundDraft ?? confirmedDraft)!.articleVersion,
              pendingChanges: !boundDraft,
              status: publishingTarget
                ? "已交接到发布任务"
                : "等待交给发布助手",
              onOpen: () => {
                if (publishingTarget) {
          ...
          ? [
              {
                ...
                title: `已确认 ${confirmedMedia.length} 家媒体`,
                description: confirmedMedia.map((item) => item.name).join("、"),
                type: "媒体清单",
                status: `选择时预估 ${formatPublishingMoney(confirmedMedia.reduce((sum, item) => sum + BigInt(item.priceTenThousandths), 0n).toString())} · 待选择稿件`,
                pendingChanges:
                  confirmedMedia
                    .map((item) => item.id)
                    .sort()
                    .join(",") !== [...selected.keys()].sort().join(","),
                onOpen: () => {
                  chooseEntry("selected");
                  setDrawerOpen(true);
                },
  ...
  const chooseArticle = (alreadySaved = false) => {
    setDrawerOpen(false);
    ...
    if (alreadySaved) acceptMediaStep("article");
    else setArticleChooserOpen(true);
  };
          ...
          outputRefs: [
            {
              resource: {
                ...
                id: saved.id,
                label: saved.articleTitle,
              },
              version: String(saved.revision),
              sourceStepId: `media-bound:${saved.id}`,
            },
          ],
        });
  ...
  const continueSelection = async () => {
    if (selectionInFlight.current || !selected.size) return;
    const generation = selectionGeneration.current;
    selectionInFlight.current = true;
    setSelectionBusy(true);
    setSelectionError("");
    try {
      if (flow) {
        const confirmed = JSON.stringify(
          selectedValues.map(({ id, name, priceTenThousandths }) => ({
            id,
            name,
            priceTenThousandths,
          })),
        );
        await (flow.saveValues ?? flow.saveSelections)({
          mediaConfirmedSelection: confirmed,
          mediaStep: draft ? "catalog" : "article",
        });
        if (generation !== selectionGeneration.current) return;
        acceptConfirmedMedia(confirmed);
      }
      if (flow && draft) {
        setBoundDraft(draft);
        setConfirmedDraft(draft);
      } else if (draft) navigate(`/publishing/drafts/${draft.id}/titles`);
      else chooseArticle(Boolean(flow));
    } catch (cause) {
      if (generation === selectionGeneration.current)
        setSelectionError("媒体清单尚未确认保存，请重试；当前选择已保留。");
    } finally {
      if (generation === selectionGeneration.current) {
        selectionInFlight.current = false;
        setSelectionBusy(false);
  ...
  const handoff = async () => {
    ...
    if (publishingTarget) {
      const url = new URL(
        projectWorkspaceUrl(`/publishing/drafts/${boundDraft.id}/titles`),
        window.location.origin,
      );
      url.searchParams.set("workbenchTask", publishingTarget.conversationId);
  ...
  return (
    ...
    >
      {flow && (
        <PublishingQuestion
          title="这次想怎样选择媒体？"
          description="先选媒体，再确定稿件。报价会在发布前再次核对。"
          value={entry}
          choices={[
            {
              value: "catalog",
              label: "浏览媒体目录",
              description: "按媒体、指标与报价挑选",
            },
            {
              value: "requirements",
              label: "按投放需求筛选",
              description: "从行业、产品和受众开始",
            },
            ...(selected.size
              ? [
                  {
                    value: "selected",
                    label: "查看已选媒体",
                    description: `继续当前 ${selected.size} 家媒体的选择`,
                  },
                ]
              : []),
          ]}
          onChoose={(next) => {
            chooseEntry(next);
            setBusinessFiltersOpen(next === "requirements");
            if (next === "selected") setDrawerOpen(true);
          }}
        />
      )}
      {(!flow || entry) && (
        <div
          className="publishing-conversation-business"
          data-reading-anchor="media-directory"
        >
          {!flow && (articleVersionId || draft) ? (
            <PublishingSteps current={2} />
          ) : null}
          ...
          {(!flow || businessFiltersOpen) && (
            ...
            >
              ...
              <button
                ...
                type="submit"
                disabled={
                  !Object.values(recommendationDraft).some((value) =>
                    value.trim(),
                  )
                }
              >
              ...
              <label className="publishing-sort-field">
                ...
                <select
                  ...
                  onChange={(event) =>
                    updateFilter(
                      "sort",
                      event.target.value as MediaFilters["sort"],
                    )
                  }
                >
                ...
                <FilterLine label="媒体推荐">
                  <div className="publishing-filter-pills">
                    ...
                    <button
                      type="button"
                      className={
                        filters.recommended === "true" ? "is-active" : ""
                      }
                      disabled={
                        recommendedCount === 0 && filters.recommended !== "true"
                      }
              ...
              <div className="publishing-advanced-grid">
                ...
                <label>
                  ...
                  >
                    ...
                    {capabilityFacetOptions(
                      facetsQuery.data?.imageSupports,
                    ).map((option) => (
                ...
                <label className="publishing-check-filter">
                  <input
                    ...
                    disabled={
                      festivalCount === 0 && filters.festival !== "true"
                    }
                    onChange={(event) =>
                      updateFilter(
                        "festival",
            ...
            <>
              ...
              >
                <div className="publishing-media-head publishing-media-row">
                  ...
                  <span className="publishing-media-col-compact-details">
                    详情
                  </span>
                </div>
                {mediaQuery.data.items.map((media) => {
                  const checked = selected.has(media.id);
                  const blocker = mediaSelectionBlocker(
                    media,
                    articleHasImages,
                  );
                      ...
                      >
                        ...
                        >
                          {media.kind === "self_media" ? (
                            <>
                              <strong>
                                粉丝 {compactNumber(media.followers)}
                              </strong>
      ...
      {flow && boundDraft ? (
        <section className="publishing-flow-result" aria-label="投放选择已保存">
          ...
          <div>
            <strong>
              {publishingTarget ? "已交接到发布任务" : "稿件与媒体已准备好"}
            </strong>
            <p>
            ...
            >
              {publishingTarget ? "打开发布任务" : "交给发布助手"}
            </button>
            <button
              type="button"
              className="publishing-button publishing-button-secondary"
      ...
      {selected.size && !boundDraft && (!flow || entry) ? (
      ...
      {drawerOpen && flow ? (
        <section
          ...
          aria-label="已选媒体明细"
          data-reading-anchor="media-selection"
        >
          <h2>这份媒体清单还需要调整吗？</h2>
          <p>
            已选 {selected.size} 家，预估合计{" "}
            {formatPublishingMoney(selectedTotal)}。最终报价以预检为准。
          </p>
          <ul className="publishing-confirmed-media-list">
            {selectedValues.map((media) => (
              <li key={media.id}>
                <span>
                  <strong>{media.name}</strong>
                  <small>{kindLabel(media.kind)}</small>
                </span>
                <span>{formatPublishingMoney(media.priceTenThousandths)}</span>
                <button
                  type="button"
                  className="publishing-button publishing-button-secondary"
                  disabled={selectionBusy}
          ...
          <div className="publishing-inline-actions">
            ...
            <button
              type="button"
              className="publishing-button publishing-button-primary"
              disabled={!selected.size || selectionBusy}
              onClick={continueSelection}
            >
              确认清单，选择稿件
            </button>
