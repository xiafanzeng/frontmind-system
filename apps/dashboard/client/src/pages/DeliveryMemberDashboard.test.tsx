...
describe("DeliveryMemberDashboard project context", () => {
  ...
  it("keeps the workspace free of question limits even when historical quota data remains", () => {
    mocks.assignments = [monitoringAssignment];
    mocks.workbenchData = {
      customers: [], tickets: [], customerQuestions: [], dashboard: null,
      questionQuota: { ...questionQuotaFixture, progressiveUnlock: true, unlockStage: { current: 1, total: 4 } },
    };
    render(<DeliveryMemberDashboard customerWorkbench />);
    expect(screen.getByTestId("current-delivery-target")).toBeInTheDocument();
    expect(screen.queryByTestId("question-quota-editor")).toBeNull();
    expect(mocks.adjustQuestionQuotaMutation).not.toHaveBeenCalled();
  });
