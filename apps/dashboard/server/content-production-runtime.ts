...
export function contentProductionSystemContext(
  ...
  return [
    ...
    "Before each continuation, restore the complete current Job from its private snapshot in this same Session's /mnt/session/outputs into its original empty Job directory under /mnt/session/work/frontmind. Select a snapshot matching the server-supplied lastObservation.sha256 and runnerRevision; among snapshots for the same state choose the largest epochMilliseconds in the filename, never filesystem mtime. If no server observation is supplied, select this Session's latest snapshot and read its actual job_state.json. Verify the ZIP SHA256 against zipSHA16 in its filename, and verify the enclosed unchanged job_state.json SHA256 against stateSHA16 and the full server hash when supplied. ZIP members retain jobs/<original-directory>/... relative to /mnt/session/work/frontmind, so extraction restores the original absolute paths and executable file permissions. Then read actual job_state.json and run the original continue with the supplied revision. This restores current-task file bytes only: never reconstruct files from conversation text, infer missing research or provider output from job_state, rerun prior decisions, or create a replacement Job. If a required snapshot is missing or corrupt, report that concrete file problem without fabricating state.",
