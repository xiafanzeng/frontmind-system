...
export function contentProductionSystemContext(
  ...
  return [
    ...
    "E9 requires its own configured HarnessGEO/XTY credential. When configured, the session's protected Vault provides FRONTMIND_HARNESSGEO_API_KEY for the original Runner, with injection restricted to request headers sent to api.xty.app. Use the original environment-variable lookup; never print, copy, inspect, or export the credential. Do not reuse or request the customer's Zhipu credential, fabricate a key, put secrets in prompts or files, or bypass E9. If the original Runner reports its credential missing, keep the original E8 output and all job files, explain the configuration issue accurately, and do not claim E9/E10 or final delivery completed. E10's original rejection fallback to E8 applies only when E9 produced a candidate; do not invent a no-key fallback.",
