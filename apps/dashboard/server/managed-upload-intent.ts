...
async function providerFileIsMissing(input: {
  ...
  try {
    const detail = await createCredentialAgentClient(input.credential, {
...
async function materializeZhipuUploadIntent(input: {
  ...
  try {
    ...
    if (error instanceof ManagedUploadCleanupRequestedError) throw error;
    const ambiguous =
    ...
    throw new ManagedUploadIntentError(
      ...
      ambiguous ? "discard_and_recreate" : "check_status",
    );
  }
  if (
