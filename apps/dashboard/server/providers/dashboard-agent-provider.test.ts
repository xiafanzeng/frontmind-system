...
describe("tenant-owned Dashboard Managed Agents transport", () => {
  ...
  it("uploads the full server-owned v4.11 archive without widening inline user uploads", async () => {
    const bytes = Buffer.alloc(29_222_014, 97);
    const attachment = {
      filename: "FrontMind_Content_Workflow_v4.11.0_Final.zip",
      mime_type: "application/zip",
      file_data: `data:application/zip;base64,${bytes.toString("base64")}`,
    };
    const f = fixture();
    await f.client({ systemAttachments: [attachment] }).createTask(request);
    const record = [...f.rows.values()].find(
      (row) => row.runtime.sessionId === "session_1",
    )!;
    expect(
    ...
    await expect(
      ordinary.client().createTask({ ...request, attachments: [attachment] }),
    ).rejects.toThrow("INLINE_FILE_TOO_LARGE");
  });
  it("sends the selected Runner action while preserving the original user message", async () => {
    const f = fixture();
    ...
    const turnContext =
      'contentProductionAction: {"kind":"confirm_competitors","revision":7}';
    await f
      .client({
        localTaskId: record.localTaskId,
        operationId: record.operationId,
        intentId: "confirm-competitors",
        turnContext,
      })
      .sendMessage({ taskId: "session_1", prompt: "确认当前比较范围" });
    const updated = f.rows.get(record.localTaskId)!;
    expect(updated.runtime.commands.at(-1)?.prompt).toBe("确认当前比较范围");
    expect(updated.runtime.commands.at(-1)?.providerPromptHash).toBe(
      digest(`确认当前比较范围\n\n${turnContext}`),
    );
  });
