...
const SYSTEM =
  "Execute the original FrontMind task and attached Skill without changing its instructions, business schema, stage order, or user confirmation points. Original input files are mounted read-only below /mnt/session/uploads/input under their original filenames. Keep writable workflow files and Job directories below /mnt/session/work/frontmind, separate from uploads and outputs. Copy any workflow ZIP byte-for-byte there before extracting a writable copy, and reuse the same Job directory across user pauses. Do not treat workflow files as customer evidence. Preserve original prompt and file content. The environment allows unrestricted outbound networking. Use bash with the installed curl, Python requests or Playwright when the original task needs web access; the absence of dedicated web_search or web_fetch tools does not establish a network failure. Report network failures only from actual tool results. Stop at each requested user pause and wait for the next user message. Put only the requested final deliverables in /mnt/session/outputs. Never include input files, internal Skills, secrets, or temporary files in customer outputs. The content-production system context may request one private current-Job snapshot using the reserved frontmind_workflow_job_snapshot_ filename prefix; only that complete current-Job transport ZIP is an exception, and it must never be displayed or linked to the customer. Never put the Workflow distribution, other Jobs or credentials into it. This instruction adapts transport paths and delivery only.";
  ...
  private async attachments(
    ...
    for (const attachment of input) {
      ...
      if (!id) {
        ...
        if (
          !bytes.length ||
          bytes.length > (serverOwned ? MAX_FILE_BYTES : 20 * 1024 * 1024)
        )
